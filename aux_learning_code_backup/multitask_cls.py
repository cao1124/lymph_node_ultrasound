...
def semi_supervised_train(data_dir, labeled_txt_path, weak_label_txt_path,
    ...
    for fold_idx in range(4):
        ...
        # 从验证集中随机抽取30%的数据加入到训练集
        if valid_list and train_list:
            all_valid_data = []
            for fold in valid_list:
                all_valid_data.extend(fold)
            sample_size = int(len(all_valid_data) * 0.8)
            sampled_data = random.sample(all_valid_data, sample_size)
            train_list[0].extend(sampled_data)
