...
@File    ：multitask_util_v2.py
@IDE     ：PyCharm 
@Author  ：cao xu
@Date    ：2025/8/18 上午10:46 
"""
import copy
import os
import random
...
class MultiTaskModel(nn.Module):
    def __init__(self, backbone, num_classes_main, num_classes_aux):
        ...
        # 添加多头分类器
        self.head_main = nn.Linear(512, num_classes_main)
        self.head_aux = nn.Linear(512, num_classes_aux)
    ...
    def forward(self, x, return_features=False):
        ...
        out_main = self.head_main(shared_features)
        out_aux = self.head_aux(shared_features)
...
img_trans = {
    'train': transforms.Compose([
        transforms.Resize([256, 256]),  # 增大尺寸保留更多信息
        transforms.RandomCrop(224),  # 随机裁剪
        transforms.RandomGrayscale(p=0.3),  # 降低灰度化概率
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.RandomVerticalFlip(p=0.5),
        transforms.RandomRotation(90),
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),  # 降低扰动强度
        transforms.ToTensor(),
    ...
    'strong': transforms.Compose([
        ...
        transforms.RandomAffine(degrees=15, translate=(0.1, 0.1)),  # 新增仿射变换
