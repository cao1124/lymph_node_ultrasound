...
import logging
import math
from backbone import MiT_encoder as MiT_encoder
from Unimiss_utils import trunc_normal_
...
class MiT(nn.Module):
    ...
    def __init__(self, norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
        ...
        if modal_type == '2D':
            ...
            self.DecEmbed2D0 = PatchEmbed2D(img_size=img_size2D // 2, patch_size=2,
                                            in_chans=self.transformer.embed_dims[-1],
            ...
            self.DecEmbed3D0 = PatchEmbed3D(img_size=[img_size3D[0] // 8, img_size3D[1] // 16, img_size3D[2] // 16],
                                            patch_size=[2, 2, 2], in_chans=self.transformer.embed_dims[-1],
                                            embed_dim=embed_dims[0])
            ...
            # 2D
            self.upsamplex2 = nn.Upsample(scale_factor=2, mode='bilinear')
            self.DecEmbed2D0 = PatchEmbed2D(img_size=img_size2D // 16, patch_size=2,
                                            in_chans=self.transformer.embed_dims[-1],
                                            embed_dim=embed_dims[0])
...
class Uni_miss_Base(nn.Module):
    def __init__(self, path, in_features, out_features):
        super().__init__()
        model = model_small()
        if path is not None:
            model.load_state_dict(torch.load(path)['student'], strict=False)
        self.image_encoder = model.transformer
        self.project = nn.Linear(in_features=in_features, out_features=in_features // 2)
        self.head = nn.Linear(in_features=in_features // 2, out_features=out_features)
        self._freeze()

    def forward(self, x, modal_type='2D'):
        out = self.image_encoder(x, modal_type=modal_type)
        cls_token =out[0][-1][:,0]
        logit = self.head(self.project(cls_token))
...
def uni_b_16(in_features, out_features):
    path = '/mnt/data/hsy/checkpoints_backup/超声预训练大模型权重/Unimiss/UniMiss_tiny.pth'
    return Uni_miss_Base(path, in_features, out_features)
