...
@File    ：multitask_cls_v2.py
@IDE     ：PyCharm 
@Author  ：cao xu
@Date    ：2025/8/18 上午10:47 
"""
from collections import Counter
from sklearn.utils import shuffle
from sklearn.metrics import confusion_matrix, classification_report
from multitask_util_v2 import *
...
def semi_supervised_train(data_dir, labeled_txt_path, weak_label_txt_path,
                          ...
                          patience, alpha=0.99, consistency_weight=1.0, ramp_up_epochs=50):
    # 1. 数据准备
    # 加载有标签数据（良恶性分类数据）
    ...
    for fold_idx in range(4):
        ...
        # 2. 数据加载器
        # 有标签数据的类权重（仅主任务） - 重点提高良性样本权重
        labeled_labels = [1 if line.strip().split(',')[1] == '恶性' else 0 for line in
                          [item for fold in train_list for item in fold]]
        label_count = Counter(labeled_labels)

        # 计算类别权重，良性样本获得3倍权重
        benign_weight = 1.0 / (label_count.get(0, 1) * 3.0)  # 良性样本3倍权重
        malignant_weight = 1.0 / (label_count.get(1, 1))
        class_weights = torch.tensor([benign_weight, malignant_weight])
        print(f"Class weights: benign={benign_weight:.4f}, malignant={malignant_weight:.4f}")

        # 采样器（仅对有标签数据） - 过采样良性样本
        if len(labeled_labels) > 0:
            # 分离良性/恶性样本索引
            benign_indices = [i for i, label in enumerate(labeled_labels) if label == 0]
            malignant_indices = [i for i, label in enumerate(labeled_labels) if label == 1]

            # 过采样良性样本使其数量等于恶性样本
            oversampled_benign = random.choices(benign_indices, k=len(malignant_indices))
            combined_labeled_indices = malignant_indices + oversampled_benign
            ...
            # 创建全局索引列表
            combined_indices = combined_labeled_indices  # 有标签部分索引 (0..labeled_count-1)

            # 添加弱标签数据索引
            weak_indices = list(range(labeled_count, labeled_count + weak_count))
            combined_indices += weak_indices
        ...
        model, optimizer, scheduler, warmup, loss_func_main, loss_func_aux = prepare_multi_task_model(
            ...
            device=device,
            weights_main=class_weights)  # 使用加权损失
        ...
        best_benign_recall = 0.0  # 跟踪最佳良性召回率
        ...
        for epoch in range(num_epochs):
            ...
            # 计算当前一致性权重（ramp-up机制）
            current_consistency_weight = consistency_weight * min(1.0, (epoch + 1) / float(ramp_up_epochs))
            print(f"Current consistency weight: {current_consistency_weight:.4f}")
            ...
            for batch in train_loader:
                ...
                if torch.any(unlabeled_mask):
                    ...
                    # 使用KL散度计算一致性损失（更适合概率分布）
                    consistency_loss = F.kl_div(
                        F.log_softmax(student_main_strong, dim=1),
                        F.softmax(teacher_main[unlabeled_mask], dim=1),
                        reduction='batchmean'
                    )
                    losses.append(current_consistency_weight * consistency_loss)
            ...
            # 5. 验证（使用教师模型）
            valid_loss, valid_main_acc, valid_benign_recall = validate(
            ...
            print(f"Val Epoch: {epoch + 1}, Loss: {valid_loss:.4f}, Acc: {valid_main_acc:.4f}, Benign Recall: {valid_benign_recall:.4f}")

            # 保存最佳模型（基于良性召回率）
            if valid_benign_recall > best_benign_recall:
                best_benign_recall = valid_benign_recall
                best_epoch = epoch + 1
                torch.save(mean_teacher.teacher.state_dict(),
                           os.path.join(pt_dir, f'fold{fold_idx}-best-benign-recall-model.pt'))
                print(f"New best benign recall model saved at epoch {best_epoch} with recall {best_benign_recall:.4f}")
        ...
        test_model.load_state_dict(torch.load(os.path.join(pt_dir, f'fold{fold_idx}-best-benign-recall-model.pt')))
        test_model.to(device)

        test_acc, test_report, test_benign_recall = evaluate(
            test_model,
            test_loader,
            device,
            target_list_main
        ...
        print(f"Test Accuracy: {test_acc:.4f}, Benign Recall: {test_benign_recall:.4f}")
        print("Classification Report:")
        print(test_report)

        # 保存测试结果
        with open(os.path.join(pt_dir, f'test_report_fold{fold_idx}.txt'), 'w') as f:
            f.write(f"Benign Recall: {test_benign_recall:.4f}\n")
            f.write(test_report)


# ================== 验证函数 ==================
def validate(model, loader, device, loss_func, target_names):
    model.eval()
    total_loss = 0.0
    correct = 0
    total = 0
    ...
    benign_recall = cm[0, 0] / (cm[0, 0] + cm[0, 1]) if (cm[0, 0] + cm[0, 1]) > 0 else 0.0

    avg_loss = total_loss / len(loader)
    accuracy = correct / total if total > 0 else 0.0

    return avg_loss, accuracy, benign_recall
...
def evaluate(model, loader, device, target_names):
    ...
    return accuracy, report, benign_recall
...
def classification():
    ...
    # 输出目录
    data = 'Section_QC/20250818-半监督-良恶性-肿大分类-优化版-'
    ...
    semi_supervised_train(
        ...
        patience=patience,
        alpha=0.999,  # 提高EMA平滑系数
        consistency_weight=1.0,  # 降低一致性损失权重
        ramp_up_epochs=50  # 添加ramp-up机制
    )
    print('训练完成')
